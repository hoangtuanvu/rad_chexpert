...
from models.attention_map import SEModule, BamBlock
from models.feature_extraction.dataset import imagenet
...
class BasicBlock(nn.Module):
    def __init__(self, inplanes, planes, stride=1, dilation=1):
        ...
        self.stride = stride  # self.se_module = SEModule(planes)
...
class BottleneckX(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, dilation=1):
        ...
        if self.use_bam:
            self.bam = BamBlock(channels=inplanes)
    ...
    def __init__(self, levels, block, in_channels, out_channels, stride=1, level_root=False,
        ...
        if levels == 1:
            ...
            self.tree1 = Tree(levels - 1, block, in_channels, out_channels, stride, root_dim=0,
                              root_kernel_size=root_kernel_size, dilation=dilation,
...
class DLA(nn.Module):
    def __init__(self, levels, channels, num_classes=1000, block=BasicBlock, residual_root=False,
                 return_levels=False, pool_size=1, linear_root=False):
        ...
        self.fc = nn.Conv2d(channels[-1], num_classes, kernel_size=1, stride=1, padding=0,
                            bias=True)
        ...
        for i in range(convs):
            modules.extend([
                nn.Conv2d(inplanes, planes, kernel_size=3, stride=stride if i == 0 else 1,
                          padding=dilation, bias=False, dilation=dilation), BatchNorm(planes),
    ...
    def load_pretrained_model(self, data_name, name):
        # assert data_name in imagenet.__dict__, \
        #     'No pretrained model for {}'.format(data_name)
        data = imagenet
        fc = self.fc
        if self.num_classes != data.classes:
            self.fc = nn.Conv2d(self.channels[-1], data.classes, kernel_size=1, stride=1, padding=0,
                bias=True)
        try:
...
def dla34(pretrained=False, **kwargs):  # DLA-34
    ...
    if pretrained:
        model.load_pretrained_model('imagenet', 'dla34')
    return model


def dla46_c(cfg, **kwargs):  # DLA-46-C
    Bottleneck.expansion = 2
    model = DLA([1, 1, 1, 2, 2, 1], [16, 32, 64, 64, 128, 256], block=Bottleneck, **kwargs)
    if cfg.pretrained:
        model.load_pretrained_model('imagenet', 'dla46_c')
    return model


def dla46x_c(cfg, **kwargs):  # DLA-X-46-C
    BottleneckX.expansion = 2
    model = DLA([1, 1, 1, 2, 2, 1], [16, 32, 64, 64, 128, 256], block=BottleneckX, **kwargs)
    if cfg.pretrained:
        model.load_pretrained_model('imagenet', 'dla46x_c')
    return model


def dla60x_c(cfg, **kwargs):  # DLA-X-60-C
    BottleneckX.expansion = 2
    model = DLA([1, 1, 1, 2, 3, 1], [16, 32, 64, 64, 128, 256], block=BottleneckX, **kwargs)
    if cfg.pretrained is not None:
        model.load_pretrained_model('imagenet', 'dla60x_c')
    return model


def dla60(cfg, **kwargs):  # DLA-60
    Bottleneck.expansion = 2
    model = DLA([1, 1, 1, 2, 3, 1], [16, 32, 128, 256, 512, 1024], block=Bottleneck, **kwargs)
    if cfg.pretrained:
        model.load_pretrained_model('imagenet', 'dla60')
    return model


def dla60x(cfg, **kwargs):  # DLA-X-60
    BottleneckX.expansion = 2
    model = DLA([1, 1, 1, 2, 3, 1], [16, 32, 128, 256, 512, 1024], block=BottleneckX, **kwargs)
    if cfg.pretrained:
        model.load_pretrained_model('imagenet', 'dla60x')
    return model


def dla102(cfg, **kwargs):  # DLA-102
    Bottleneck.expansion = 2
    model = DLA([1, 1, 1, 3, 4, 1], [16, 32, 128, 256, 512, 1024], block=Bottleneck,
                residual_root=True, **kwargs)
    if cfg.pretrained:
        model.load_pretrained_model('imagenet', 'dla102')
    return model


def bam_dla102x(pretrained=False, **kwargs):  # DLA-X-102
    BottleneckX.expansion = 2
    model = DLA([1, 1, 1, 3, 4, 1], [16, 32, 128, 256, 512, 1024], block=BottleneckX,
                residual_root=True, **kwargs)
    if pretrained:
        model.load_pretrained_model('imagenet', 'dla102x')
    return model


def dla102x2(cfg, **kwargs):  # DLA-X-102 64
    BottleneckX.cardinality = 64
    model = DLA([1, 1, 1, 3, 4, 1], [16, 32, 128, 256, 512, 1024], block=BottleneckX,
                residual_root=True, **kwargs)
    if cfg.pretrained:
        model.load_pretrained_model('imagenet', 'dla102x2')
    return model


def dla169(cfg, **kwargs):  # DLA-169
    Bottleneck.expansion = 2
    model = DLA([1, 1, 2, 3, 5, 1], [16, 32, 128, 256, 512, 1024], block=Bottleneck,
                residual_root=True, **kwargs)
    if cfg.pretrained:
        model.load_pretrained_model('imagenet', 'dla169')
    return model