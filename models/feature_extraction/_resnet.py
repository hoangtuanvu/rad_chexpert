...
__all__ = ['ResNet', 'BasicBlock',
           'Bottleneck']  # model_registry will add each entrypoint fn to this


def _cfg(url='', **kwargs):
    return {'url': url, 'num_classes': 1000, 'input_size': (3, 224, 224), 'pool_size': (7, 7),
            'crop_pct': 0.875, 'interpolation': 'bilinear', 'mean': IMAGENET_DEFAULT_MEAN,
            'std': IMAGENET_DEFAULT_STD, 'first_conv': 'conv1', 'classifier': 'fc', **kwargs}


default_cfgs = {'resnet18': _cfg(url='https://download.pytorch.org/models/resnet18-5c106cde.pth'),
                'resnet34': _cfg(
                ...
                'resnet26': _cfg(
                    ...
                    interpolation='bicubic'), 'resnet26d': _cfg(
        url='https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-weights/resnet26d-69e92c46.pth',
        interpolation='bicubic'), 'resnet50': _cfg(
        url='https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-weights/resnet50_ram-a26f946b.pth',
        interpolation='bicubic'), 'resnet50d': _cfg(url='', interpolation='bicubic'),
                ...
                'resnext50_32x4d': _cfg(
                    ...
                    interpolation='bicubic'), 'resnext50d_32x4d': _cfg(
        url='https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-weights/resnext50d_32x4d-103e99f8.pth',
        interpolation='bicubic'), 'resnext101_32x4d': _cfg(url=''), 'resnext101_32x8d': _cfg(
        url='https://download.pytorch.org/models/resnext101_32x8d-8ba56ff5.pth'),
                'resnext101_64x4d': _cfg(url=''), 'tv_resnext50_32x4d': _cfg(
        url='https://download.pytorch.org/models/resnext50_32x4d-7cdf4587.pth'),
                ...
                'seresnext26d_32x4d': _cfg(
                    ...
                    interpolation='bicubic'), 'seresnext26t_32x4d': _cfg(
        url='https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-weights/seresnext26t_32x4d-361bc1c4.pth',
        interpolation='bicubic'), 'seresnext26tn_32x4d': _cfg(
        url='https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-weights/seresnext26tn_32x4d-569cb627.pth',
        interpolation='bicubic'), 'ecaresnext26tn_32x4d': _cfg(url='', interpolation='bicubic'),
                'ecaresnet18': _cfg(), 'ecaresnet50': _cfg(), }
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, cardinality=1, base_width=64,
                 reduce_first=1, dilation=1, first_dilation=None, act_layer=nn.ReLU,
        ...
        self.conv1 = nn.Conv2d(inplanes, first_planes, kernel_size=3, stride=stride,
                               padding=first_dilation, dilation=first_dilation, bias=False)
        self.bn1 = norm_layer(first_planes)
        self.act1 = act_layer(inplace=True)
        self.conv2 = nn.Conv2d(first_planes, outplanes, kernel_size=3, padding=dilation,
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, cardinality=1, base_width=64,
        ...
        self.conv2 = nn.Conv2d(first_planes, width, kernel_size=3, stride=stride,
                               padding=first_dilation, dilation=first_dilation, groups=cardinality,
                               bias=False)
...
def downsample_conv(in_channels, out_channels, kernel_size, stride=1, dilation=1,
    ...
    return nn.Sequential(*[
        nn.Conv2d(in_channels, out_channels, kernel_size, stride=stride, padding=p,
                  dilation=first_dilation, bias=False), norm_layer(out_channels)])
...
def downsample_avg(in_channels, out_channels, kernel_size, stride=1, dilation=1,
    ...
    return nn.Sequential(
        *[pool, nn.Conv2d(in_channels, out_channels, 1, stride=1, padding=0, bias=False),
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, in_chans=3, cardinality=1, base_width=64,
                 stem_width=64, stem_type='', block_reduce_first=1, down_kernel_size=1,
                 avg_down=False, output_stride=32, act_layer=nn.ReLU, norm_layer=nn.BatchNorm2d,
                 drop_rate=0.0, drop_path_rate=0., drop_block_rate=0., global_pool='avg',
        ...
        if deep_stem:
            ...
            self.conv1 = nn.Sequential(
                *[nn.Conv2d(in_chans, stem_chs_1, 3, stride=2, padding=1, bias=False),
                  norm_layer(stem_chs_1), act_layer(inplace=True),
                  nn.Conv2d(stem_chs_1, stem_chs_2, 3, stride=1, padding=1, bias=False),
                  norm_layer(stem_chs_2), act_layer(inplace=True),
        ...
        layer_kwargs = dict(reduce_first=block_reduce_first, act_layer=act_layer,
                            norm_layer=norm_layer, avg_down=avg_down,
                            down_kernel_size=down_kernel_size, drop_path=dp, **block_args)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1, reduce_first=1,
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample_args = dict(in_channels=self.inplanes, out_channels=planes * block.expansion,
                                   kernel_size=down_kernel_size, stride=stride, dilation=dilation,
                                   first_dilation=first_dilation,
                                   norm_layer=kwargs.get('norm_layer'))
            downsample = downsample_avg(**downsample_args) if avg_down else downsample_conv(
                **downsample_args)

        block_kwargs = dict(cardinality=self.cardinality, base_width=self.base_width,
                            reduce_first=reduce_first, dilation=dilation, **kwargs)
        layers = [block(self.inplanes, planes, stride, downsample, first_dilation=first_dilation,
                        **block_kwargs)]
...
def resnet26d(pretrained=False, num_classes=1000, in_chans=3, **kwargs):
    ...
    model = ResNet(Bottleneck, [2, 2, 2, 2], stem_width=32, stem_type='deep', avg_down=True,
...
def resnet50d(pretrained=False, num_classes=1000, in_chans=3, **kwargs):
    ...
    model = ResNet(Bottleneck, [3, 4, 6, 3], stem_width=32, stem_type='deep', avg_down=True,
                   num_classes=num_classes, in_chans=in_chans, **kwargs)
    model.default_cfg = default_cfg
    if pretrained:
        load_pretrained(model, default_cfg, num_classes, in_chans)
...
def wide_resnet50_2(pretrained=False, num_classes=1000, in_chans=3, **kwargs):
    ...
    """
    model = ResNet(Bottleneck, [3, 4, 6, 3], base_width=128, num_classes=num_classes,
...
def wide_resnet101_2(pretrained=False, num_classes=1000, in_chans=3, **kwargs):
    ...
    """
    model = ResNet(Bottleneck, [3, 4, 23, 3], base_width=128, num_classes=num_classes,
                   in_chans=in_chans, **kwargs)
...
def resnext50_32x4d(pretrained=False, num_classes=1000, in_chans=3, **kwargs):
    ...
    model = ResNet(Bottleneck, [3, 4, 6, 3], cardinality=32, base_width=4, num_classes=num_classes,
                   in_chans=in_chans, **kwargs)
    model.default_cfg = default_cfg
    if pretrained:
        load_pretrained(model, default_cfg, num_classes, in_chans)
...
def resnext50d_32x4d(pretrained=False, num_classes=1000, in_chans=3, **kwargs):
    ...
    model = ResNet(Bottleneck, [3, 4, 6, 3], cardinality=32, base_width=4, stem_width=32,
                   stem_type='deep', avg_down=True, num_classes=num_classes, in_chans=in_chans,
                   **kwargs)
    model.default_cfg = default_cfg
    if pretrained:
        load_pretrained(model, default_cfg, num_classes, in_chans)
...
def resnext101_32x4d(pretrained=False, num_classes=1000, in_chans=3, **kwargs):
    ...
    model = ResNet(Bottleneck, [3, 4, 23, 3], cardinality=32, base_width=4, num_classes=num_classes,
                   in_chans=in_chans, **kwargs)
    model.default_cfg = default_cfg
    if pretrained:
        load_pretrained(model, default_cfg, num_classes, in_chans)
...
def resnext101_32x8d(pretrained=False, num_classes=1000, in_chans=3, **kwargs):
    ...
    model = ResNet(Bottleneck, [3, 4, 23, 3], cardinality=32, base_width=8, num_classes=num_classes,
                   in_chans=in_chans, **kwargs)
    model.default_cfg = default_cfg
    if pretrained:
        load_pretrained(model, default_cfg, num_classes, in_chans)
...
def resnext101_64x4d(pretrained=False, num_classes=1000, in_chans=3, **kwargs):
    ...
    """
    default_cfg = default_cfgs['resnext101_32x4d']
    model = ResNet(Bottleneck, [3, 4, 23, 3], cardinality=64, base_width=4, num_classes=num_classes,
                   in_chans=in_chans, **kwargs)
    model.default_cfg = default_cfg
    if pretrained:
        load_pretrained(model, default_cfg, num_classes, in_chans)
...
def ig_resnext101_32x8d(pretrained=True, **kwargs):
    ...
    if pretrained:
        load_pretrained(model, num_classes=kwargs.get('num_classes', 0),
...
def seresnext26d_32x4d(pretrained=False, num_classes=1000, in_chans=3, **kwargs):
    ...
    model = ResNet(Bottleneck, [2, 2, 2, 2], cardinality=32, base_width=4, stem_width=32,
                   stem_type='deep', avg_down=True, num_classes=num_classes, in_chans=in_chans,
...
def seresnext26t_32x4d(pretrained=False, num_classes=1000, in_chans=3, **kwargs):
    ...
    model = ResNet(Bottleneck, [2, 2, 2, 2], cardinality=32, base_width=4, stem_width=32,
                   stem_type='deep_tiered', avg_down=True, num_classes=num_classes,
...
def seresnext26tn_32x4d(pretrained=False, num_classes=1000, in_chans=3, **kwargs):
    ...
    model = ResNet(Bottleneck, [2, 2, 2, 2], cardinality=32, base_width=4, stem_width=32,
                   stem_type='deep_tiered_narrow', avg_down=True, num_classes=num_classes,
                   in_chans=in_chans, block_args=dict(attn_layer='se'), **kwargs)
    model.default_cfg = default_cfg
    if pretrained:
        load_pretrained(model, default_cfg, num_classes, in_chans)
...
def ecaresnet18(pretrained=False, num_classes=1000, in_chans=3, **kwargs):
    ...
    block_args = dict(attn_layer='eca')
    model = ResNet(BasicBlock, [2, 2, 2, 2], num_classes=num_classes, in_chans=in_chans,
