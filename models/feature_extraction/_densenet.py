...
import pathlib
import os
from models.feature_extraction.utils import load_state_dict_from_url
...
class DenseNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=(6, 12, 24, 16), num_init_features=64,
                 norm_type='Unknown', bn_size=4, drop_rate=0, num_classes=18, use_se=True):
        ...
        'conv0', nn.Conv2d(1, num_init_features, kernel_size=7, stride=2, padding=3, bias=False)),
            ('norm0', get_norm(norm_type, num_init_features)), ('relu0', nn.ReLU(inplace=True)),
...
def _load_torchxrayvision_models(weights, model):
    ...
    weights_storage_folder = os.path.expanduser(
        os.path.join("~", "PycharmProjects/cxr/torchxrayvision"))
...
def _densenet(arch, growth_rate, block_config, num_init_features, pretrained, progress, norm_type,
              use_se, **kwargs):
    model = DenseNet(growth_rate, block_config, num_init_features, norm_type=norm_type,
                     use_se=use_se, **kwargs)
    if pretrained:
        _load_torchxrayvision_models(arch, model)
...
def densenet121(cfg, progress=True, **kwargs):
    ...
    """
    return _densenet(cfg.xray_type, 32, (6, 12, 24, 16), 64, cfg.pretrained, progress,
                     norm_type='BatchNorm', use_se=False, **kwargs)
...
def densenet161(cfg, progress=True, **kwargs):
    ...
    return _densenet('densenet161', 48, (6, 12, 36, 24), 96, cfg.pretrained, progress,
                     norm_type=cfg.norm_type, use_se=cfg.use_se, **kwargs)
