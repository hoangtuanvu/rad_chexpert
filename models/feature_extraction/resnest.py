...
import torch
from models.feature_extraction.s_resnet import ResNet, Bottleneck
...
def resnest50(cfg, root='~/.encoding/models', **kwargs):
...
def resnest101(cfg=None, root='~/.encoding/models', **kwargs):
...
def resnest200(cfg=None, root='~/.encoding/models', **kwargs):
...
def resnest269(cfg=None, root='~/.encoding/models', **kwargs):
    model = ResNet(Bottleneck, [3, 30, 48, 8], radix=2, groups=1, bottleneck_width=64,
                   deep_stem=True, stem_width=64, avg_down=True, avd=True, avd_first=False,
                   use_se=cfg.use_se, **kwargs)
    if cfg.pretrained:
        model.load_state_dict(
            torch.hub.load_state_dict_from_url(resnest_model_urls['resnest269'], progress=True),
            strict=False)
    return model
